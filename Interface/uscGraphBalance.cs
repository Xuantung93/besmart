        ...
        private void nudCriteria_ValueChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (var item in values)
                {
                    ...
                    {
                        Business.ManagementDataBase.tabelaSmartNorm[item.Key] = (float)nudCriteria.Value;
                    }
                    else
                    {
                        Business.ManagementDataBase.tabelaSmartNorm[item.Key] = Business.ManagementDataBase.tabelaSmartNorm[item.Key] - (diff / (Business.ManagementDataBase.tabelaSmartNorm.Count - 1));
                    }
                }
                FillBalance();
            }
